...
- **Broadcast endpoint on amtechai.com, built on existing AMTECH certs (not a new format):** `scripts/signing/sign-anchor.ts` emits an `amtech-signed-artifact/v2` **status** certificate (`subjectType:'status'`, `subjectId:'authority-sth'`) whose subject is `sth.json` — binds its SHA-256/SHA3-512, timestamped (`issuedAt`=STH timestamp), Ed25519-signed by the domain key, optional PGP co-sign (`AMTECH_ANCHOR_PGP_KEY`, skipped without gpg/key). Served at `/.well-known/authority/anchor/certificate.json(.sig)`. **Verifiable with the EXISTING `artifact:verify`** (`valid:true`), gated in `validate-skills`, surfaced on `/registry`. Wired into `skills:sign` (after sign-authority). Idempotent.
