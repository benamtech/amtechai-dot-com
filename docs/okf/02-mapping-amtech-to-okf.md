...
3. **Entities are inline, not concepts.** ~~`primaryEntity`/`entities[]` are attributes of articles, not standalone nodes, so the entity graph is implicit.~~ **Resolved (2026-06-19):** the connective Service / Method / Outcome / Tool / Problem entities are now first-class concept nodes in `src/lib/knowledge/entities.ts` (alongside the derived Use Case / Place / Industry nodes). Articles resolve to them by name via `deriveNamedEntityEdgeIds`, so each entity is its own document with article + cross-entity backlinks. Use Case / Place / Industry remain field-derived. Remaining inline strings are one-off, article-specific entities not worth promoting.
